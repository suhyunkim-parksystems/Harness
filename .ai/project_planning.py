  ...
  "features": [
    {
      ...
      "reuse_targets": [
        {"path": "재사용 플랫폼 파일 맵에 있는 실제 경로(맵에 있는 그대로)", "symbol": "가져다 쓸 클래스/모듈/인터페이스 이름(특정 못 하면 \\"\\")", "how_to_use": "이 feature에서 어떻게 상속/등록/호출해 재사용하는지 1문장"}
      ],
      "reference_targets": [
        {"path": "참고 레퍼런스 파일 맵에 있는 실제 경로(맵에 있는 그대로)", "symbol": "참고할 타입/메서드/패턴 이름(특정 못 하면 \\"\\")", "why": "이 feature 구현에 무엇을 참고하는지 1문장"}
      ],
      "no_reuse_reason": "재사용 플랫폼이 주어졌는데 reuse_targets가 비었으면 이 feature가 왜 플랫폼 자산을 안 쓰는지 한 문장(쓰면 \\"\\")",
      "no_reference_reason": "참고 레퍼런스가 주어졌는데 reference_targets가 비었으면 그 이유 한 문장(아니면 \\"\\")"
    }
  ]
}
...
- 기획서에 없는 큰 기능을 임의로 추가하지 않는다.
- (재사용 플랫폼 섹션이 있을 때만) 각 feature는 reuse_targets에, 그 feature가 실제로 가져다 쓸 플랫폼 파일을 1개 이상 '파일 맵에 있는 실제 경로 그대로' 적는다. 정말 쓸 게 없으면 reuse_targets는 []로 두고 no_reuse_reason에 이유를 적는다. 경로를 지어내지 마라.
- (참고 레퍼런스 섹션이 있을 때만) 각 feature는 reference_targets에, 구현에 참고할 형제 앱 파일을 1개 이상 '파일 맵에 있는 실제 경로 그대로' 적는다. 참고할 게 없으면 []로 두고 no_reference_reason에 이유를 적는다. 경로를 지어내지 마라.
- 재사용/참고 섹션이 아예 없으면 reuse_targets/reference_targets는 []로 둔다.
- JSON 밖에 설명 문장을 쓰지 않는다.
""".strip()
...
#  (2) agy는 dot-디렉터리(.ai/.project/...)를 hidden으로 보고 워크스페이스에서 무시한다
#      (cli.log "failed to add ... is hidden: ignore uri"). -> 출력 파일은 dot-dir 밖, 비숨김 경로
...
def build_reuse_section(reuse_folders: list[Path]) -> str:
    ...
    out: list[str] = [
        ...
        "- 그리고 그 구체 자산을 각 feature의 reuse_targets(path/symbol/how_to_use)에 '파일 맵의 실제 경로 그대로' 채워라. 경로를 지어내면 거부된다.",
...
def build_references_section(reference_folders: list[Path]) -> str:
    ...
    out: list[str] = [
        ...
        "- 각 feature가 구현에 참고할 형제 앱 파일을 reference_targets(path/symbol/why)에 '파일 맵의 실제 경로 그대로' 채워라. 경로를 지어내면 거부된다.",
...
# --------------------------------------------------------------------------- #
# 참조 타깃 강제 (reference/reuse 구체 인용 강제)
#
# --reference-folder / --reuse-folder 가 주어지면, 각 feature가 "구체적으로 어떤
# 파일을 참조/재사용하는지"를 구조적 필드(reuse_targets/reference_targets)에 적게 하고,
# 그 경로가 실제로 존재하는지 폴더와 대조해 검증한다(환각 경로 차단). 위반이 있으면
# 모델에 repair를 재요청하고, 끝내 못 채우면 PlanningError로 중단한다. 마지막으로 검증된
# 타깃을 refined_prompt 본문에 합성해, 파일 맵을 못 보는 다운스트림 harness run 에이전트가
# 실제로 열 수 있는 경로를 받게 한다.
# --------------------------------------------------------------------------- #

MAX_TARGET_REPAIR_ROUNDS = 2


def _norm_cited(value: Any) -> str:
    """모델이 적은 경로 문자열을 정규화한다(역슬래시->슬래시, 따옴표/공백 제거)."""
    return str(value or "").replace("\\", "/").strip().strip('"').strip("'").strip()


def _canonical_target_file(cited: str, folders: list[Path]) -> Path | None:
    """모델이 적은 경로가 주어진 폴더 안의 '실재 파일'이면 resolved Path를, 아니면 None.

    절대경로 / repo-루트 상대 / 폴더 상대 / 폴더명 포함 표기를 모두 시도하되,
    최종적으로 resolved 경로가 폴더 안에 있어야만 인정한다(밖을 가리키면 거부).
    """
    raw = _norm_cited(cited)
    if not raw:
        return None
    attempts: list[Path] = []
    candidate = Path(raw)
    if candidate.is_absolute():
        ...
        attempts.append(ROOT / raw)
        for folder in folders:
            attempts.append(folder / raw)
            attempts.append(folder.parent / raw)  # 모델이 폴더명을 경로에 포함한 경우
    for attempt in attempts:
        ...
        for folder in folders:
            try:
                froot = folder.resolve()
            except OSError:
                continue
            if resolved == froot or froot in resolved.parents:
                return resolved
    return None


def _normalize_target_entries(raw: Any) -> list[dict[str, Any]]:
    """모델이 준 *_targets 값을 dict 리스트로 정규화한다(문자열만 준 경우도 허용)."""
...
def _feature_label(feature: dict[str, Any]) -> str:
    return str(
        feature.get("_id")
        or feature.get("slug")
        or feature.get("_slug")
        or feature.get("title")
        or "feature"
    )


def _validate_feature_targets(
    ...
    *,
    ...
    """한 feature의 한 종류(재사용/참고) 타깃을 검증한다.

    검증된 타깃을 feature[cache_key]에 canonical 형태로 채우고 (hard, soft) 위반을 돌려준다.
      - hard: 실재하지 않는(환각) 경로. repair/중단을 유발한다.
      - soft: 타깃이 비었고 사유도 없는 경우. 경고만 남기고 진행한다(배치를 막지 않음).
    """
    label = _feature_label(feature)
    entries = _normalize_target_entries(feature.get(targets_key))
    valid: list[dict[str, str]] = []
    hard: list[str] = []
    soft: list[str] = []
    for entry in entries:
        cited = _norm_cited(entry.get("path"))
        if not cited:
            continue
        resolved = _canonical_target_file(cited, folders)
        if resolved is None:
            hard.append(
                f"[{label}] {kind} 타깃 경로가 파일 맵에 없거나 실재하지 않습니다: {cited!r} "
                "(파일 맵에 있는 실제 경로를 그대로 쓰세요)"
            )
            continue
        valid.append(
            {
                ...
                "note": str(entry.get(note_key) or "").strip(),
            }
        )
    feature[cache_key] = valid
    if not valid and not str(feature.get(reason_key) or "").strip():
        soft.append(
            f"[{label}] {kind} 타깃이 비어 있고 사유({reason_key})도 없습니다. "
            f"참조/재사용할 게 있으면 {targets_key}에 적고, 정말 없으면 {reason_key}에 한 문장으로 사유를 적으세요."
        )
    return hard, soft


def validate_decomposition_targets(
    features: list[dict[str, Any]],
    *,
    reference_folders: list[Path],
    reuse_folders: list[Path],
) -> tuple[list[str], list[str]]:
    """모든 feature의 reuse/reference 타깃을 검증한다.

    검증된 타깃을 각 feature에 (_reuse_targets/_reference_targets)로 캐시하고
    (hard, soft) 위반 전체를 돌려준다(hard=환각 경로, soft=빈 타깃·사유 없음).
    """
    hard: list[str] = []
    soft: list[str] = []
    for feature in features:
        if not isinstance(feature, dict):
            continue
        if reuse_folders:
            feature_hard, feature_soft = _validate_feature_targets(
                feature,
                kind="재사용 플랫폼",
                folders=reuse_folders,
                targets_key="reuse_targets",
                reason_key="no_reuse_reason",
                note_key="how_to_use",
                cache_key="_reuse_targets",
            )
            hard.extend(feature_hard)
            soft.extend(feature_soft)
        if reference_folders:
            feature_hard, feature_soft = _validate_feature_targets(
                feature,
                kind="참고 레퍼런스",
                folders=reference_folders,
                targets_key="reference_targets",
                reason_key="no_reference_reason",
                note_key="why",
                cache_key="_reference_targets",
            )
            hard.extend(feature_hard)
            soft.extend(feature_soft)
    return hard, soft
...
def _render_feature_targets_block(feature: dict[str, Any]) -> str:
    """검증된 타깃을 refined_prompt에 붙일 한국어 블록으로 만든다(없으면 "")."""
    reuse = feature.get("_reuse_targets") or []
    reference = feature.get("_reference_targets") or []
    if not reuse and not reference:
        return ""
    lines = ["", "[참조 지점 - 구현 전 반드시 먼저 이 파일들을 열어 확인할 것]"]
    if reuse:
        lines.append("재사용(아래 자산을 새로 만들지 말고 그대로 상속/등록/호출):")
        for target in reuse:
            symbol = f" ({target['symbol']})" if target.get("symbol") else ""
            note = f" - {target['note']}" if target.get("note") else ""
            lines.append(f"- {target['path']}{symbol}{note}")
    if reference:
        lines.append("참고(패턴/조립 방식만 참고, 통째로 복제 금지):")
...
def apply_targets_to_features(features: list[dict[str, Any]]) -> None:
    """검증된 타깃 블록을 각 feature의 refined_prompt 본문에 1회 합성한다.

    refined_prompt가 다운스트림 harness run의 유일한 입력 채널이므로, 파일 맵을 못 보는
    run 에이전트가 실제로 열 수 있는 경로를 여기서 본문에 박아 전달한다.
    """
    for feature in features:
        if not isinstance(feature, dict):
            continue
        block = _render_feature_targets_block(feature)
        if not block:
            continue
        base = str(
            feature.get("refined_prompt")
            or feature.get("description")
            or feature.get("title")
            or ""
        ).strip()
        feature["refined_prompt"] = (base + "\n" + block).strip()


TARGET_REPAIR_INSTRUCTION = """
## 참조 타깃 교정 (필수)
위 분해 결과에 아래 문제가 있다. 재사용 플랫폼/참고 레퍼런스의 '파일 맵'을 다시 보고, 각 feature가
실제로 참조/재사용할 파일을 그 맵에 있는 '실제 경로 그대로' 채워 교정하라.
- reuse_targets/reference_targets의 path는 위 파일 맵에 있는 경로여야 한다. 경로를 지어내지 마라.
- 정말 참조/재사용할 게 없는 feature만 해당 배열을 []로 두고 no_reuse_reason/no_reference_reason에 이유를 적어라.
- 다른 필드(slug/depends_on/tier/performance 등)는 바꾸지 말고 유지하라.
...
def build_target_repair_prompt(
    *,
    request: str,
    plan_files: list[Path],
    reference_folders: list[Path],
    ...
    """현재 분해 결과 + 파일 맵 + 발견된 위반을 묶어 타깃 교정용 프롬프트를 만든다."""
    ...
    current = json.dumps(decomposition, ensure_ascii=False, indent=2)[:CANDIDATE_MAX_CHARS]
    violation_text = "\n".join(f"- {item}" for item in violations[:40])
    instruction = TARGET_REPAIR_INSTRUCTION.replace("{violations}", violation_text)
    return (
        base
        + "\n\n## 교정 대상 분해 결과 (현재본)\n```json\n"
        + current
...
def enforce_decomposition_targets(
    harness: Any,
    args: argparse.Namespace,
    *,
    decomposition: dict[str, Any],
    ...
    """reference/reuse 폴더가 주어졌을 때 각 feature의 구체 참조 타깃을 강제한다.

    검증 -> 위반 시 모델에 repair 재요청(최대 MAX_TARGET_REPAIR_ROUNDS) -> 끝내 위반이면
    PlanningError. 통과하면 검증된 타깃을 refined_prompt에 합성해 분해 결과를 돌려준다.
    폴더가 없으면 강제 대상이 없으므로 그대로 반환한다(하위 호환).
    """
    if not reference_folders and not reuse_folders:
        return decomposition

    features = [f for f in (decomposition.get("features") or []) if isinstance(f, dict)]
    hard, soft = validate_decomposition_targets(
        features, reference_folders=reference_folders, reuse_folders=reuse_folders
    )
    # repair 루프는 '하드'(환각/실재하지 않는 경로)만 구동한다. soft(빈 타깃·사유 없음)는
    # 배치를 막지 않으므로, 하드 때문에 repair가 도는 김에 모델이 같이 채우도록 프롬프트에만 동봉한다.
    rounds = 0
    while hard and rounds < MAX_TARGET_REPAIR_ROUNDS:
        rounds += 1
        print(
            f"참조 타깃 환각/오류 {len(hard)}건 - 모델에 교정 재요청 ({rounds}/{MAX_TARGET_REPAIR_ROUNDS})",
            file=sys.stderr,
        )
        prompt = build_target_repair_prompt(
            request=request,
            plan_files=plan_files,
            reference_folders=reference_folders,
            reuse_folders=reuse_folders,
        ...
        repaired = plan_model_json(
            harness,
            args,
            prompt=prompt,
            logs_dir=logs_dir,
            log_prefix=f"target_repair_{rounds}",
            repair_schema=DECOMPOSE_SCHEMA,
            status_message=f"참조 타깃 교정 중 ({rounds}/{MAX_TARGET_REPAIR_ROUNDS})",
        )
        repaired_features = repaired.get("features")
        if isinstance(repaired_features, list) and repaired_features:
            decomposition = repaired
            features = [f for f in repaired_features if isinstance(f, dict)]
        hard, soft = validate_decomposition_targets(
            features, reference_folders=reference_folders, reuse_folders=reuse_folders
        )

    if hard:
        detail = "\n".join(f"  - {item}" for item in hard[:20])
        more = f"\n  ... 외 {len(hard) - 20}건" if len(hard) > 20 else ""
        raise PlanningError(
            "참조 타깃 강제 실패: 실재하지 않는(환각) 경로를 모델이 끝내 교정하지 못했습니다.\n"
            + detail
            + more
        )
    if soft:
        print(
            f"경고: 참조/재사용 타깃 없이 진행하는 feature {len(soft)}건(사유 미기재). "
            "정말 참조가 필요 없는 feature면 정상입니다. plan.txt에서 확인하세요.",
            file=sys.stderr,
        )
        for item in soft[:20]:
            print(f"  - {item}", file=sys.stderr)

    apply_targets_to_features(features)
    return decomposition
...
def _run_one_blind_draft(
    ...
    *,
    ...
    """한 drafter의 블라인드 초안을 1회 실행한다. (drafter, 유효초안|None, 드롭사유|None).

    동시 실행 워커로 호출되므로 콘솔 스피너는 끈다(배치 스피너 하나만 쓴다). 예외는 잡아
    드롭 사유로 변환해, 한 provider가 실패해도 다른 초안을 죽이지 않는다.
    """
    try:
        candidate = plan_model_json(
            ...
            repair_schema=DECOMPOSE_SCHEMA,
            status_message=f"독립 초안 작성 중 ({drafter})",
            show_spinner=False,
        )
    except di.InterviewError as exc:
        return drafter, None, str(exc)
    except Exception as exc:  # noqa: BLE001 - 워커 예외가 풀을 죽이지 않게 사유로 변환.
        return drafter, None, f"예기치 못한 오류: {exc}"
    features = candidate.get("features")
    if isinstance(features, list) and features:
        return drafter, candidate, None
    return drafter, None, "features 배열 없음"
...
def _collect_blind_drafts(
    ...
    *,
    ...
    """drafter들을 '동시에'(서로 안 보게) 돌려 (유효 초안, 성공한 provider)를 모은다.

    블라인드 독립성은 실행 순서가 아니라 프롬프트에서 나온다(각 drafter가 같은 draft_prompt만
    받고 서로의 결과를 못 봄). provider별 로그/agy 캐시 경로가 분리되고 subprocess 대기 중 GIL이
    풀리므로 ThreadPoolExecutor로 병렬화한다(deep_thinking의 _run_drafts_parallel과 동일 원리).
    결과는 drafter 순서로 재정렬해 결정성을 유지한다. parallel·max가 공유하며, 실패/무효 초안은
    경고만 남기고 건너뛴다.
    """
    total = len(drafters)
    if total == 0:
    ...
    order = {drafter: index for index, drafter in enumerate(drafters)}
    results: list[tuple[str, dict[str, Any] | None, str | None]] = []
    # 동시 실행 동안 per-draft 스피너 대신 배치 스피너 하나만 콘솔에 띄운다(tty가 아니면 자동 no-op).
    spinner = (
        di.Spinner(f"독립 초안 {total}개 동시 작성 중 ({', '.join(drafters)})")
        if total > 1
        else di.Spinner(f"독립 초안 작성 중 ({drafters[0]})")
    )
    with spinner:
        with concurrent.futures.ThreadPoolExecutor(max_workers=total) as pool:
            futures = [
                pool.submit(
                    _run_one_blind_draft,
                    harness,
                    args,
    ...
    candidates: list[dict[str, Any]] = []
    succeeded: list[str] = []
    for drafter, candidate, error in results:
        if candidate is not None:
            candidates.append(candidate)
            succeeded.append(drafter)
        else:
            print(f"  초안 드롭({drafter}): {error} - 건너뜀", file=sys.stderr)
...
def _run_agy_via_file(
    ...
    *,
    ...
    try:
        with (di.Spinner(status_message) if show_spinner else contextlib.nullcontext()):
...
# project_planning의 tier(중요도/위험 사다리)를 orchestrate.py batch.json의 실행 필드로 사상한다.
# tier_to_invocation과 같은 정의를 orchestrate 스키마(pipeline/deep_thinking/strategy)로 번역한 것이다:
#   fast/standard/full     -> deep_thinking 없음(strategy는 스키마 기본값 parallel로 둠, 미사용)
#   full-parallel/-max 등   -> pipeline=full + deep_thinking=True + 해당 strategy
TIER_TO_ORCHESTRATE = {
    "fast": ("fast", False, "parallel"),
    "standard": ("standard", False, "parallel"),
    "full": ("full", False, "parallel"),
    "full-parallel": ("full", True, "parallel"),
    "full-sequential": ("full", True, "sequential"),
    "full-max": ("full", True, "max"),
}


def tier_to_orchestrate(tier: Any) -> tuple[str, bool, str]:
    """tier를 orchestrate batch.json의 (pipeline, deep_thinking, strategy)로 사상한다.

    알 수 없는 tier는 tier_to_invocation과 동일하게 full(심층 없음)로 떨어뜨린다.
    """
    normalized = str(tier or "full").strip().lower()
    return TIER_TO_ORCHESTRATE.get(normalized, ("full", False, "parallel"))


# orchestrate.py batch.json의 비-티어 기본값. orchestrate.HARD_DEFAULTS / batch.example.json과 일치시킨다.
BATCH_COOLDOWN_SECONDS = 900
...
def feature_request_text(feature: dict[str, Any]) -> str:
    """run 요청 본문. refined_prompt -> description -> title -> slug 순으로 폴백한다.

    plan.txt의 명령(feature_command)과 batch.json(build_batch_record)이 같은 요청문을 쓰도록 공유한다.
    """
    return (
        feature.get("refined_prompt")
        or feature.get("description")
...
def feature_command(feature: dict[str, Any]) -> str:
    ...
    prompt = feature_request_text(feature)
    parts = [
        "python",
        script,
        "run",
...
def render_plan_text(
    *,
    ...
    for feature in ordered:
        ...
        for target in feature.get("_reuse_targets") or []:
            symbol = f" ({target['symbol']})" if target.get("symbol") else ""
            lines.append(f"  재사용 : {target['path']}{symbol}")
        for target in feature.get("_reference_targets") or []:
            symbol = f" ({target['symbol']})" if target.get("symbol") else ""
            lines.append(f"  참고   : {target['path']}{symbol}")
        lines.append("")
...
def fresh_run_state() -> dict[str, Any]:
    """orchestrate가 채워 나갈 run 1개의 초기 state(전부 미실행)."""
    return {
        "status": "pending",
        "attempts": 0,
        "stage": None,
...
def build_batch_record(*, project_slug: str, ordered: list[dict[str, Any]]) -> dict[str, Any]:
    """orchestrate.py가 그대로 실행할 수 있는 batch.json 레코드를 만든다.

    각 feature를 run 1개로 펼친다. tier는 orchestrate 스키마의
    (pipeline, deep_thinking, strategy)로 사상하고, feature id/요청문/performance는
    plan.txt와 동일하게 맞춘다. batch.example.json과 같은 톤으로 모든 필드를 run마다
    명시하고, state는 항상 pending으로 둔다(실행 이력은 orchestrate가 기록).
    """
    runs: list[dict[str, Any]] = []
    for feature in ordered:
        pipeline, deep_thinking, strategy = tier_to_orchestrate(feature.get("tier"))
        runs.append(
            {
                "feature": feature["_id"],
                "request": di.compact_text(feature_request_text(feature)),
                "pipeline": pipeline,
                ...
                "strategy": strategy,
                "timeout": BATCH_DEFAULTS["timeout"],
                "max_steps": BATCH_DEFAULTS["max_steps"],
                "max_verify_fix_retries": BATCH_DEFAULTS["max_verify_fix_retries"],
                "max_attempts": BATCH_DEFAULTS["max_attempts"],
                "state": fresh_run_state(),
            }
        )
    return {
        "name": project_slug,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", help="출력 디렉터리 override. 기본은 .project/projects/<project-slug>/.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # reference/reuse 폴더가 있으면 각 feature가 구체적으로 무엇을 참조/재사용하는지
        # 강제하고(환각 경로 차단), 검증된 타깃을 refined_prompt 본문에 합성한다.
        decomposition = enforce_decomposition_targets(
            harness,
            args,
            decomposition=decomposition,
            request=request,
            ...
            "features": [
                {
                    ...
                    "reuse_targets": feature.get("_reuse_targets") or [],
                    "reference_targets": feature.get("_reference_targets") or [],
        ...
        # orchestrate.py가 그대로 먹을 수 있는 batch.json을 plan.txt와 같은 디렉터리에 만든다.
        batch_record = build_batch_record(project_slug=project_slug, ordered=ordered)
        ...
        print()
        print("또는 orchestrate로 일괄 자동 실행(실패 분류·재시도·성공 시 자동 푸시):")
        print(f"  python .ai/orchestrate.py run {_display_path(batch_path)}")
